...
case $yn in
    [Yy]* )
        ...
        clear
        echo "Running Node-RED $EXTRAW for user $USER at $HOME on $MYOS\n"
        time1=$(date)
        ...
        if curl -f https://raw.githubusercontent.com/node-red/raspbian-deb-package/master/resources/node-red-icon.svg >/dev/null 2>&1; then
